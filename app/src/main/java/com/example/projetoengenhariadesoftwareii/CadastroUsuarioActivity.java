...
public class CadastroUsuarioActivity extends AppCompatActivity {
    ...
    private void salvarUsuario() {
        try {
            ...
            String orcamentoStr = spOrcamento.getSelectedItem().toString();
            int objetivoId = getIdFromResposta(objetivo, new String[]{"Emagrecer", "Ganho de Massa", "Manter Peso", "Melhorar Saúde"});
            int atividadeId = getIdFromResposta(atividade, new String[]{"Sedentário", "Leve", "Moderado", "Intenso"});
            int praticidadeId = getIdFromResposta(praticidade, new String[]{"Baixa", "Moderada", "Alta"});
            int rigorId = getIdFromResposta(rigor, new String[]{"Leve", "Moderado", "Rigoroso"});
            orcamentoStr = orcamentoStr.replace("R$", "").trim();
            ...
            usuario.setObjetivoId(objetivoId);
            usuario.setAtividadeId(atividadeId);
            usuario.setPraticidadeId(praticidadeId);
            usuario.setRigorId(rigorId);
    ...
    // 🔹 AGORA ESTÁ FORA DO MÉTODO (FUNCIONA!)
    private int getIdFromResposta(String resposta, String[] opcoes) {
        for (int i = 0; i < opcoes.length; i++) {
            if (resposta.equalsIgnoreCase(opcoes[i])) return i + 1;
        }
        return 0;
    }
